...
const columns = ref([
    ...
    },
    {
        ...
        value: (row) => {
            if (!Array.isArray(row.permissions)) return "N/A";
            const permissions = row.permissions.map((p) => p.name);
            if (permissions.length <= 5) {
                return permissions.join(", ");
            }
            return `${permissions.slice(0, 5).join(", ")} and ${
...
// Custom Button Mapping Function
