...
const modelName = "supplier-invoices";
...
const columns = ref([
    {
        ...
        uri: (row) => route("supplier-invoices.show", row.id),
    ...
    },
    {
        ...
        uri: (row) => route("suppliers.show", row.supplier.id),
        class: "text-green-600 hover:underline",
        icon: "mdi-account-outline",
    },
    {
        label: "Total Amount",
        value: (row) => formatNumber(row.total_amount, {
            style: "currency",
            currency: "PHP",
        }),
    },
